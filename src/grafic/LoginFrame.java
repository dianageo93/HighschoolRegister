...
import grafic.utils.Frame;
import grafic.utils.PasswordField;
import grafic.utils.TextField;
import grafic.utils.UserFrame;
			...
			public void actionPerformed(ActionEvent e) {
				if(getUsername().equals(USERNAME_TEXTFIELD)) {
					...
					if(myAuthHelper.loginAccepted(getUsername(), getPassword())) {
						...
						else if(myAuthHelper.getTip() == TipUtilizator.SECRETAR) {
							SecretarFrame mySecretarFrame = new SecretarFrame();
							mySecretarFrame.setUsername(getUsername());
							mySecretarFrame.setWhatUser("Secretar");
							mySecretarFrame.setUsernameLabel();
							mySecretarFrame.setupHomePanel();
						}
						dispose();
					}
					else {
